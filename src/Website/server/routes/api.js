...
const getUserGuilds = async (sesKey) => {
    ...
    for (var i = 0; i < userCurrentGuilds.length; i++) {
        userCurrentGuilds[i].permissions = new Permissions(userCurrentGuilds[i].permissions_new).toArray();
        ...
        if (userCurrentGuilds[i].icon) userCurrentGuilds[i].icon = `https://cdn.discordapp.com/icons/${userCurrentGuilds[i].id}/${userCurrentGuilds[i].icon}`
    }
...
route.get('/commands', (req, res) => {
    if (!commandsCache || commandsCache.length == 0) {
        ...
        categories.forEach(category => {
            ...
            const categoryCommandFiles = fs.readdirSync(path.join(__dirname, '..', '..', '..', 'Commands', category)).filter(file => file.endsWith('.js'));
            categoryCommandFiles.forEach(categoryCommandFile => {
                let categoryCommand; 
                try {
                    categoryCommand = new (require(path.join(__dirname, '..', '..', '..', 'Commands', category, categoryCommandFile)))();
                }
                catch(err) {
                    return;
                }

                if (!categoryCommand.name || !categoryCommand.required_perms || !categoryCommand.required_bot_perms) return;
...
route.get('/users/@me', (req, res) => {
    if (!req.cookies?.session) return res.status(400).json({ status: 400, message: 'Bad Request' }).end();
    else {
        const data = encryptor.decrypt(req.cookies.session);
        if (!data) return res.status(400).json({ status: 400, message: 'Bad Requet' }).end();
        delete data["key"];
...
route.get('/users/:id', (req, res) => {
    if (!req.params?.id || req.params?.id.length != 18 || isNaN(req.params.id)) return res.status(400).json({ status: 400, message: 'Bad Requst' }).end();

    return res.status(503).json({ status: 503, message: 'Service Unavailable' }).end();
})

route.get('/users/@me/guilds', async (req, res) => {
    if (!req.cookies?.session) return res.status(400).json({ status: 400, message: 'Bad Request' }).end();

    const session = await encryptor.decrypt(req.cookies.session);

    if (!session?.key) return res.status(400).json({ status: 400, message: 'Bad Request' }).end();
    ...
    if (req?.query?.selectManageable?.toLowerCase() == 'true') {
        return res.status(200).json({
            status: 200,
            data: currentGuilds.filter(x => x.owner == true || x.permissions.includes('ADMINISTRATOR'))
        }).end();
    }
...
// Guilds Routes
route.all('/guilds/:id', async (req, res) => {
    if (!req.params?.id || req.params?.id.length != 18 || isNaN(req.params.id)) return res.status(400).json({ status: 400, message: 'Bad Request' }).end();

    if (!req.cookies?.session) return res.status(401).json({ status: 401, message: 'Unauthorized' }).end();

    const session = encryptor.decrypt(req.cookies.session);

    if (!session.key) return res.status(401).json({ status: 401, message: 'Unauthorized' }).end();

    if (req.method == 'GET') {

        const userGuilds = (await getUserGuilds(session.key)).filter(x => x.owner == true || x.permissions.includes('ADMINISTRATOR'));
        ...
        if (!canAccess) return res.status(401).json({ status: 401, message: 'Unauthorized' }).end();
    
        try {
    
            const guildData = (await Athena.dbManager.getGuild(req.params.id, true)).data || null;
    
            res.status(200).json({ status: 200, data: guildData || null }).end();
        }
        catch(err) {
        ...
        if (!req.body || !req.body.guild || !req.body?.prefix || !req.body?.language) return res.status(400).json({ status: 400, message: 'Bad Request' }).end();
        ...
        try {
            ...
            Athena.dbManager.updateDocument('guild', req.body.guild, { $set: { "data.preferences.prefix": req.body.prefix } });
            Athena.dbManager.updateDocument('guild', req.body.guild, { $set: { "data.preferences.language": req.body.language } });
        }
        catch(err) {
            Athena.log(2, err);
            res.status(500).json({ status: 500, message: 'Server Error' }).end();
