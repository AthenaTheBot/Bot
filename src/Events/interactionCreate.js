module.exports = class Event {
    ...
    async run(client, interaction) {
        ...
        const guildData = await client.dbManager.getGuild(interaction.guild.id, true);
        const userData = await client.dbManager.getUser(interaction.member.id, true);
        ...
        if (guildData?.data && userData?.preferences) {
            ...
            if (command) {
                ...
                let userCanRun = true;
                const userPerms = interaction.member.permissionsIn(interaction.channel).toArray();
                command.required_perms.forEach(perm => {
                    if (!userPerms.includes(perm)) userCanRun = false;
                });

                // * Perm Checkup
                if (!botCanRun) return interaction.reply(mainLocale.BOT_NOT_ENOUGH_PERM);
                ...
                if (command.category == 'Music') {
                    ...
                    if (msg.guild.me.voice?.channel && interaction?.guild?.me?.voice?.channel?.id != interaction?.member?.voice.channel?.id) return interaction.reply(mainLocale.NOT_SAME_VC);

                    if (command.name != 'play' && command.name != 'join' &&  command.name != 'disconnect') {
                        if (!guildState || !guildState.playing) return interaction.reply(mainLocale.ALREADY_NOT_PLAYING_SONG);
                    }
                }
