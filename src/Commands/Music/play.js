...
class Command extends BaseCommand {
  ...
  async run(client, msg, args, locale) {
    ...
    if (!requestedSong)
      return msg.reply({ embeds: [Embed.setDescription(locale.INVALID_SONG)] });
    ...
    if (guildState.playing) {
      ...
      if (!queueSong)
        return msg.reply({
          embeds: [Embed.setDescription(locale.SONG_NOT_FOUND)],
        });
      ...
      return msg.reply({
        embeds: [
          Embed.setDescription(
            locale.SONG_ADDED_TO_QUEUE.replace(
              "$song",
              `[${queueSong.title}](${queueSong.url})`
            )
          ),
        ],
      });
  ...
  async getSong(reqSong) {
    ...
    if (!type) {
      const ytSearchResult = await playDL.search(reqSong, {
        limit: 1,
        type: "video",
      });
      if (ytSearchResult[0]) {
        return new Track(
          ytSearchResult[0].url,
          ytSearchResult[0].title,
          ytSearchResult[0].channel.name,
          ytSearchResult[0].durationInSec
        );
      } else {
        return null;
      }
      ...
      if (type == "sp_track") {
        ...
        if (spotifyTrackData) {
          const artistNames = [];
          spotifyTrackData.artists.forEach((artist) =>
            artistNames.push(artist.name)
          );
          const ytSearchResult = await playDL.search(
            `${artistNames.join(" ")} ${spotifyTrackData.name}`,
            { limit: 1, type: "video" }
          );
          if (ytSearchResult[0]) {
            return new Track(
    ...
    async function startedPlaying(playing, guildState) {
      ...
      if (!playing) msg.reply({ embeds: [Embed.setDescription(locale.ERROR)] });
        ...
        await msg
          .reply({
            embeds: [
              Embed.setColor("#5865F2").setDescription(
                guildState.locale.NOW_PLAYING.replace(
                  "$song",
                  `[${guildState.queue[0].title}](${guildState.queue[0].url})`
                )
              ),
            ],
          })
