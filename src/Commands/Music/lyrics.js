const run = async (client, message, args, locale, db) => {
    ...
    const search = await fetch(`http://api.musixmatch.com/ws/1.1/track.search?q=${song}&page_size=1&page=1&s_track_rating=desc&apikey=${client.config.api.MUSIXMATCH}`).then(res => res.json());
   ...
   const result = await fetch(`http://api.musixmatch.com/ws/1.1/track.lyrics.get?track_id=${id}&apikey=${client.config.api.MUSIXMATCH}`).then(res => res.json());
