...
class Command extends BaseCommand {
    constructor(){
        super({
            ...
            required_bot_perms: ["SEND_MESSAGES", "READ_MESSAGE_HISTORY", "BAN_MEMBERS"]
        });
    }
    ...
    async run(client, msg, args, locale) {
        ...
        if (mentionedMember.id === msg.author.id) return msg.reply({ embeds: [ Embed.setDescription(`${locale.CANNOT_BAN_YOURSELF}`) ] });
    
        if (mentionedMember.id === client.user.id) return msg.reply({ embeds: [ Embed.setDescription(`${locale.CANNOT_BAN_MYSELF}`) ] });
        ...
        if (rawPos1 > rawPos2 || msg.author.id === msg.guild.ownerID || !mentionedMember.bannable) {
    
            if (rawPos3 < rawPos2) return msg.reply({ embeds: [ Embed.setDescription(`${locale.BOT_NOT_ENOUGH_PERMS}`) ] });
    
            try {

                msg.guild.members.cache.get(mentionedMember.id).ban({ reason: reason });
            } catch (err) {
